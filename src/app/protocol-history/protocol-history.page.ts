...
export class ProtocolHistoryPage {
    ...
    loadProtocol() {
        this.apiService.pvs4_get_protocol_history(this.idCustomer, this.idProduct).then((result: any) => {
           ...
           let nodes: any[] = JSON.parse(JSON.stringify(result.list));
           for (let i = 0; i < nodes.length; i++) {            
                let prdct = JSON.parse(nodes[i].data.product);
                this.removeRow = 0;
    ...
    protocolPage(prtcl: any[]) {
        ...
        for (let index = 0; index < prtcl.length; index++) {
            ...
            for (let i = 0; i < options.length; i++) {
                ...
                if(options[i].type == 5) prtcl[index].data[options[i].title[this.lang]] = pipe.transform(options[i].value,'dd.MM.yyyy');
                if(options[i].type != 5) prtcl[index].data[options[i].title[this.lang]] = options[i].value;   
            }
    ...
    printPdf() {
        ...
        for (var x = 0, lenX = pageCount; x < lenX; x++) {
            // Header
            headerArray = [];
            widthsArray = [];
            for (var k = 0, len = HeaderCount+1; k < len; k++) {
                headerArray.push({ text:'.', style: 'header' });
                widthsArray.push("*");
            }
       
            // Body
            for (var i = 0, lenA = this.cols.length; i < lenA; i++) {
                colArray.push(this.cols[i].header);
                let colCnt: any = 0;
                for (var j = colCountStart+colCountAdd, lenB = colCountEnd+colCountAdd; j < lenB; j++) {
                ...
                if(colCnt < HeaderCount) {
                    for (var e = 0, lenE = HeaderCount-colCnt; e < lenE; e++) {
                        colArray.push('');
                    }
                }
