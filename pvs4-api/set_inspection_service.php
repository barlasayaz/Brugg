...
function processing($id) {
    ...
    $id   = intval( trim( mysqli_escape_string($con,$id) ));
    $mode = intval( trim( mysqli_escape_string($con,$_POST['mode']) ) );
    //if( ($mode!=0) && ($mode!=1) ) $mode=0;
    
    $sql_1    = "UPDATE `products` SET `inspection_service`=$mode  WHERE  `customer`=$id;";
