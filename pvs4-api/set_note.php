...
function processing() {
    ...
    $contact_person = intval( $_POST['contact_person'] );
    $notes          = trim( mysqli_escape_string($con,$_POST['notes']) );
    $notes_date     = trim( mysqli_escape_string($con,$_POST['notes_date']) );
    ...
    if($id >0) {
        ...
        $sql="INSERT INTO notes (customer, user, title, contact_person, notes,  notes_date, active,  category) 
                         VALUES ($customer, $user, '$title', $contact_person, '$notes', '$notes_date', $active,  $category)";
    }
