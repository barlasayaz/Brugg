...
function processing() {
    ...
    $all= mysqli_query($con,"SELECT * FROM profiles  ;");
    if($all) {             
        if(mysqli_num_rows($all) > 0){
            ...
            while ($row = mysqli_fetch_assoc($all)) {
                if(strlen($row['last_name'])<=0 ){
                    ...
                    if($response->code==200){
                        $fn = $response->body->obj->first_name;
                        $ln = $response->body->obj->last_name;
                        $id = $row['id'];
                        $sql = "UPDATE `profiles` SET `first_name` = '$fn', `last_name` = '$ln' WHERE `profiles`.`id` = $id;";
                        echo $sql.'  ';
