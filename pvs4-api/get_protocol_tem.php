...
function processing( ) {
    ...
    $view      = intval ( trim( mysqli_escape_string($con, $_POST['view']) ) ); // 0=all; 1=licensee; 2=only own

    // todo: user, licensee and view filter
    
    $sql    = "SELECT * FROM `protocol_templates`  WHERE `licensee`=$licensee AND  `active`=1 ;";
