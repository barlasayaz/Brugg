...
function processing() {
    ...
    $type  = intval ( trim( mysqli_escape_string($con,$_POST['type']) ) ); // 1=customer 2=products
    $rev  = intval ( trim( mysqli_escape_string($con,$_POST['rev']) ) );
    ...
    if($type==1){
        $sql    = "SELECT * FROM `customer` where (`rev` > $rev) AND (`licensee`=1) ORDER BY `rev`;";
        $ret_sql= mysqli_query( $con, $sql );
    } else  if($type==2){
        $sql    = "SELECT * FROM `products` where (`rev` > $rev) ORDER BY `rev`;";
        $ret_sql= mysqli_query( $con, $sql );
        //todo filter nach licensee
    } else{
        http_response_code(500);
        $error = new \stdClass();
